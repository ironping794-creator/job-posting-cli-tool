...
DERIVED_FIELDS = ["salary_min", "salary_max", "salary_unit", "matched_keywords", "is_target_city", "is_salary_known", "dedupe_key"]
OUTPUT_FIELDS = list(FIELD_ALIASES.keys()) + DERIVED_FIELDS
...
def clean_rows(rows: list[dict[str, str]]) -> tuple[list[dict[str, str]], int]:
    ...
    for row in rows:
        ...
        dedupe_key = "|".join([item["detail_url"].lower(), item["title"].lower(), item["company"].lower(), item["city"].lower()])
...
def filter_rows(rows: list[dict[str, str]], cities: list[str], keywords: list[str], salary_min: float | None) -> list[dict[str, str]]:
    selected = []
    for row in rows:
        ...
        if city_ok and keyword_ok and salary_ok:
            item = dict(row)
            item["matched_keywords"] = ",".join(matched)
            item["is_target_city"] = "yes" if city_ok else "no"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Clean, filter, and summarize recruitment posting CSV files.")
    ...
    parser.add_argument("--xlsx", action="store_true", help="Also write formatted XLSX workbooks.")
    return parser
...
def run(args: argparse.Namespace) -> None:
    ...
    if args.xlsx:
        write_xlsx(out_dir / "cleaned_jobs.xlsx", cleaned, OUTPUT_FIELDS, "Cleaned Jobs")
        write_xlsx(out_dir / "filtered_jobs.xlsx", filtered, OUTPUT_FIELDS, "Filtered Jobs")
